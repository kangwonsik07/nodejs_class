...
router.post('/', isLoggedIn, upload.single('img'), async (req, res) => {
   try {
      ...
      if (!req.file) {
         // 업로드된 파일이 없거나 무언가 이상이 생겨서 파일 정보가 넘어오지ㅐ 않는 경우
      ...
      // 추출된 해시태그가 있으면
      if (hashtags) {
         //   Promise.all: 여러개의 비동기 작업을 병렬로 처리. 모든 해시태그가 데이터 베이스에서 생성되거나 찾아질때까지 기다름
         //   병렬 처리 : 동시에 여러개의 작업을 실행
         /*
            Hashtag.findocCreate({where: {title: '여행}})
            Hashtag.findocCreate({where: {title: '맛집}})

            두 작업이 동시에 실행됨 -> 장점, 속도가 빨라짐, 
          */
         ...
         result=[
          [HashTagOnstance1,true] // #여행 해시태그가 새로 생성됨(false 라면 이미 존재하는 해시태그)
          [HashTagOnstance1,true] // #맛집 해시태그가 새로 생성됨
           ]  
          
          r[0] = HashTagOnstance1
          r[0] = HashTagOnstance2
           */
         // addHashtahs(): HashTagOnstance를 통해 post와의 관계를 설정하고 이 과정에서 postHashtag테이블의 postId와 hashtagId 칼럼에 값이 추가됨
         await post.addHashthags(result.map((r) => r[0]))
      }
...
router.put('/:id', isLoggedIn, upload.single('img'), async (req, res) => {
   try {
      ...
      // 게시물에서 해시태그를 추출해서 존재하는 해시태그는 유지하고 새로운 해시태그는 넣어준다
      const hashtags = req.body.hashtags.match(/#[^\s#]*/g) // #을 기준으로 해시태그 추출
