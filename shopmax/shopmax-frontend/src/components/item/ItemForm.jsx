...
import { formatwithComma, stripComma } from '../../utils/PliceSet'
...
function ItemForm({ onSubmit, initialValues = {} }) {
   ...
   const handleImageChange = useCallback((e) => {
      ...
      // 업로드 된 파일 객체를 배열로 바꾸고 최대 5개 까지만 선택
      ...
      const newImgUrls = newFiles.map((file) => {
         ...
         // promise 객체를 리턴하여 비동기적으로 fileReader 작업 완료(속도가 빠름)
         return new Promise((resolve) => {
            // 파일 올리기 완료 되었을 때 결과를 promise에 전달
            reader.onload = (event) => {
               // event.target.reslut: 인코딩 된 이미지 미리보기 url이 들어있음
               resolve(event.target.result)
            }
         })
      })

      // 모든 파일 데이터 url을 생성 완료한 후 실행
   ...
   const handlePriceChange = useCallback((e) => {
      ...
      // price 컬럼의 타입은 int(정수형)이므로 콤마가 붙은 값을 insert 할수 없으므로 price state의 값은 콤마를 제거해 저장한다.
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ mt: 3 }} encType="multipart/form-data">
         ...
         <FormControl fullWidth sx={{ mt: 2 }}>
            ...
            <Select labelId="item-sell-status-label" label="판매상태" value={itemSellStatus} onChange={(e) => setItemSellStatus(e.target.value)}>
               {/* value는 실제 itemSellStatus 칼럼에 저장될 값 */}
