...
module.exports = (server, sessionMiddleware) => {
   ...
   const io = new Server(server, {
      // 소켓에서 별도로 cors설정: socket.IO와 Express의 cors처리 방식이 다르기 때문
      cors: {
         origin: process.env.FRONTEND_APP_URL, // 클라이언트의 url 허용
         methods: ['GET', 'POST'], // 허용된 http 메서드, 다른 메서드(put,delete,patch)는 차단(클라이언드가 delete요총을 보내 데이터가 삭제되는 상황 방지)
   ...
   io.on('connection', (socket) => {
      ...
      // 클라이언트에서 'user info'이벤트 요청시 사용자 정보 클라이언트에게 전송
      ...
      socket.on('disconnect', () => {
         console.log('사용자 연결 해제:', user?.id) // 연결해제된 사용자의 id출력
