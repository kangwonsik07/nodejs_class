...
// 주문 목록(페이징) localhost:8000//order/list?page=1&limit=5&startDate=2025-01-01&endDate=2025-01-16
router.get('/list', verifyToken, isLoggedIn, async (req, res) => {
   try {
      ...
      const endDateTime = `${endDate} 23:59:59` // 년월일만 받을시 시분초는 00:00:00으로 인식하므로 시간변경(endDate 날짜에 주문한 내용도 검색되도록)
         ...
         include: [
            {
               ...
               // 교차테이블 데이터(OrderItem 테이블에서 필요한 칼럼 선택)
               through: {
...
// 주문취소 localhost:8000/order/cancel/:id
router.post('/cancel/:id', verifyToken, isLoggedIn, async (req, res) => {
   ...
   try {
      ...
      // 주문내역이 없다면
      if (!order) {
         return res.status(404).json({ success: false, message: '주문 내역이 존재하지 않습니다.' })
      }
      // 이미 취소된 주문이라면
...
// 주문삭제 localhost:8000/order/delete/:id
